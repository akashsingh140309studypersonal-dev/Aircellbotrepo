...
# Exact Code Registry map for dynamic edit pre-views
COMMAND_CODE_REGISTRY = {
    "addfeature": "async def addfeature_start(update, context):\n    if not is_owner(update.effective_user.id): return ConversationHandler.END\n    await update.message.reply_text('📝 Send code to append...')",
    "editfeature": "async def editfeature_start(update, context):\n    if not is_owner(update.effective_user.id): return ConversationHandler.END\n    # Display dynamic options",
    "recover": "async def recover_user(update, context):\n    matches = re.findall(r'(\\d{7,12})\\s*[-:|~]*\\s*@?([a-zA-Z0-9_]+)?', text)\n    # Bulk DB insertion logic",
    "giveowner": "async def give_owner(update, context):\n    new_owner = int(context.args[0])\n    OWNERS.append(new_owner)",
    "add": "async def add_user_cmd(update, context):\n    cursor.execute('INSERT OR REPLACE INTO users (user_id, username, full_name) VALUES (?, ?, ?)', (uid, uname, name))\n    conn.commit()",
    ...
    "users": "async def users_list(update, context):\n    cursor.execute('SELECT user_id, full_name, username FROM users')\n    # Output member details",
    "admins": "async def admins_list(update, context):\n    cursor.execute('SELECT admins.user_id, users.username, users.full_name FROM admins LEFT JOIN users ON admins.user_id = users.user_id')",
    "giveadmin": "async def give_admin(update, context):\n    cursor.execute('INSERT OR IGNORE INTO admins (user_id) VALUES (?)', (uid,))",
    "removeadmin": "async def remove_admin(update, context):\n    cursor.execute('DELETE FROM admins WHERE user_id = ?', (uid,))",
    "postchannel": "async def post_channel_manager(update, context):\n    cursor.execute('INSERT OR IGNORE INTO channels (channel_id, title) VALUES (?, ?)', (ch_input, ch_input))",
    "recent": "async def recent_cmd(update, context):\n    await history_list(update, context, limit=10)",
    "allhistory": "async def allhistory_cmd(update, context):\n    await history_list(update, context, limit=None)",
    "resetall": "async def resetall_confirm(update, context):\n    cursor.execute('UPDATE users SET points = 0, diamonds = 0, attempts = 0, correct = 0, incorrect = 0')",
    "newquiz": "async def new_quiz_start(update, context):\n    # Initiates photo/text MCQ & Numeric flow",
    "upmaterials": "async def upmaterials_start(update, context):\n    # Save study documents to database",
    "apvmaterial": "async def apvmaterial_list(update, context):\n    # Approve/Reject requested user notes",
    "removematerial": "async def remove_material_cmd(update, context):\n    cursor.execute('DELETE FROM materials WHERE id = ?', (int(target),))",
    "cancel": "async def cancel(update, context):\n    context.user_data.clear()\n    return ConversationHandler.END",
    "myprofile": "async def my_profile(update, context):\n    await view_profile(update, context)",
    "profile": "async def profile_command(update, context):\n    await view_profile(update, context, target_uid=uid)",
    ...
    "materials": "async def materials_list(update, context):\n    cursor.execute('SELECT DISTINCT title FROM materials')",
    "reqmaterial": "async def reqmaterials_start(update, context):\n    # Material request sequence"
}
...
async def editfeature_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(all_cmds), 2):
        row = []
        row.append(InlineKeyboardButton(f"⚙️ /{all_cmds[i]}", callback_data=f"editcmd_{all_cmds[i]}"))
        if i + 1 < len(all_cmds):
            row.append(InlineKeyboardButton(f"⚙️ /{all_cmds[i+1]}", callback_data=f"editcmd_{all_cmds[i+1]}"))
        buttons.append(row)
    
    await update.message.reply_text("🛠 **Select Command Module to Edit:**\nChoose any command code logic to view its python source and override it:", reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")
...
async def editfeature_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code_snippet = COMMAND_CODE_REGISTRY.get(cmd, f"# Module logic for /{cmd}")
    ...
    msg_text = (
        f"📜 **Current Code Logic Preview for `/{cmd}`:**\n"
        f"```python\n{code_snippet}\n```\n\n"
        f"📌 Now send the **NEW Python code** to override and execute this command logic:"
    )
...
async def editfeature_save_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open("custom_features.py", "a", encoding="utf-8") as f:
        f.write(f"\n# Module /{cmd}\n" + code + "\n")
    execute_custom_code()
    await update.message.reply_text(f"✅ Code for `/{cmd}` saved and applied live successfully!", parse_mode="Markdown")
    return ConversationHandler.END
...
async def post_channel_manager(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        cursor.execute("INSERT OR IGNORE INTO channels (channel_id, title) VALUES (?, ?)", (ch_input, ch_input))
        conn.commit()
        await update.message.reply_text(f"✅ Added channel `{ch_input}` to posting list!", parse_mode="Markdown")
        return
    ...
    if not channels:
        ...
        for ch_id, title in channels:
            msg += f"• `{ch_id}`\n"
            buttons.append([InlineKeyboardButton(f"🗑 Remove {ch_id}", callback_data=f"remch_{ch_id}")])
...
async def select_posting_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = []
    for ch_id, title in channels:
        buttons.append([InlineKeyboardButton(f"📢 {title} ({ch_id})", callback_data=f"pubch_{ch_id}")])
    
    if len(channels) > 1:
        buttons.append([InlineKeyboardButton("🌐 Post in ALL Registered Channels", callback_data="pubch_ALL")])

    await update.message.reply_text("📢 **Where would you like to post this Question?**\nSelect target channel:", reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")
...
async def publish_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for target_chat in target_channels:
        quiz_id = f"q{int(datetime.datetime.now().timestamp())}_{target_chat}"
