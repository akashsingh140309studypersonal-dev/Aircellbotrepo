...
async def editfeature_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg_text = (
        f"📜 **Current Code Logic for `/{cmd}`:**\n"
        f"```python\n{code_snippet[:1500]}\n```\n\n"
        f"📌 Now send the **NEW Python code** to update this command:"
    )
...
async def editfeature_save_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open("custom_features.py", "w", encoding="utf-8") as f:
        f.write(f"# Updated module /{cmd}\n" + code + "\n")
    execute_custom_code()
    await update.message.reply_text(f"✅ Code for `/{cmd}` replaced and executed live successfully!", parse_mode="Markdown")
    return ConversationHandler.END
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    menu_text = "📖 ━━━━ **BOT COMMANDS HELP** ━━━━ 📖\n\n"

    if is_owner(user_id):
        menu_text += (
            "👑 **OWNER CONTROLS**\n"
            "• `/giveowner <ID>` - Grant Owner\n"
            "• `/giveadmin` / `/removeadmin` - Manage Admins\n"
            "• `/addfeature` / `/editfeature` - Dynamic Code\n"
            "• `/recover` - Smart Recover Members\n"
            "• `/givepts` / `/removepts` - Modify Points\n"
            "• `/postchannel` - Manage Multi-Channel Output\n"
            "• `/recent` / `/allhistory` - Quiz History\n"
            "• `/resetall` - Reset All Database\n\n"
        )
    if is_admin(user_id):
        menu_text += (
            "🛠 **ADMIN CONTROLS**\n"
            "• `/newquiz` - Create MCQ/Multi/Numeric Quiz\n"
            "• `/users` - Registered members count\n"
            "• `/upmaterials` - Upload Study Material\n"
            "• `/apvmaterial` - Approve Material Requests\n"
    ...
    menu_text += (
        ...
        "• `/profile <ID/@user>` - Inspect user profile\n"
        "• `/top` - Top 30 Leaderboard\n"
        "• `/materials` - Download Study Notes\n"
        "• `/reqmaterial` - Request New Material\n"
    )
