...
# Admin / Owner Management Functions (Fixed /users)
...
async def publish_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if quiz['type'] == "MCQ":
        ...
        if row:
            buttons.append(row)
        buttons.append([InlineKeyboardButton("💬 Attempt in Bot Direct", url=f"https://t.me/{bot_username}?start=quiz_{quiz_id}")])
        ...
        buttons = [
            ...
            [InlineKeyboardButton("💬 Attempt in Bot Direct", url=f"https://t.me/{bot_username}?start=quiz_{quiz_id}")]
        ]
        markup = InlineKeyboardMarkup(buttons)
