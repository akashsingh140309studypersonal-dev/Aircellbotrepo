...
async def setup_bot_commands(app: Application):
    ...
    admin_commands = member_commands + [
        ...
        BotCommand("track", "📍 Track Question Analytics (48h)"),
...
async def get_help_text(user_id, user_name):
    ...
    if is_admin(user_id):
        menu_text += (
            ...
            "🛠 ❯ /track • Track Active/Past Question Analytics\n"
...
def build_quiz_markup(quiz_id, q_type, options, selections=None):
    ...
    if q_type == "MCQ":
        ...
        for k, v in options.items():
            icon = "🟢" if k in selections else ""
            prefix_label = f"{icon} " if icon else ""
            row.append(InlineKeyboardButton(f"{prefix_label}{k}: {v}", callback_data=f"multi#{quiz_id}#{k}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
...
async def publish_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for target_chat in target_channels:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (quiz_id, user.id, user.full_name, str(target_chat), text_to_send[:50], now_time, 'pending', skip_lb))
        conn.commit()
...
async def declare_leaderboard(context: ContextTypes.DEFAULT_TYPE):
    ...
    q_data = active_quizzes[quiz_id]
    dec_time = datetime.datetime.now()

    cursor.execute("UPDATE quiz_history SET status='declared', declared_at=? WHERE quiz_id=?", (dec_time, quiz_id))
    conn.commit()

    if q_data.get('skip_lb'):
        q_data['status'] = 'declared'
        q_data['declared_at'] = dec_time
        return
...
# ADMIN QUESTION TRACKING FEATURE (/track)
async def track_questions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("DELETE FROM quiz_history WHERE posted_at < ?", (cutoff_time,))
    conn.commit()

    to_del = [qid for qid, qd in active_quizzes.items() if qd.get('posted_at', datetime.datetime.now()) < cutoff_time]
    for qid in to_del:
        del active_quizzes[qid]

    if is_owner(user_id):
        cursor.execute("SELECT quiz_id, posted_at, status FROM quiz_history WHERE posted_at >= ? ORDER BY posted_at DESC", (cutoff_time,))
    else:
        cursor.execute("SELECT quiz_id, posted_at, status FROM quiz_history WHERE admin_id = ? AND posted_at >= ? ORDER BY posted_at DESC", (user_id, cutoff_time))

    rows = cursor.fetchall()
    if not rows:
        await update.message.reply_text("📌 No posted questions within last 48 hours.")
        return

    buttons = []
    for qid, posted_at, status in rows:
        dt_str = str(posted_at)[:19]
    ...
    await update.message.reply_text("📊 **Select Posted Question Timing to view Detailed Audit Report:**", reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")
...
async def track_details_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rep_time_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    status_str = f"✅ Declared on: `{declared_at}`" if status == 'declared' else f"⏳ Live (Report as of: `{rep_time_str}`)"

    report = (
        f"📊 **QUESTION TRACKING REPORT**\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 **Posted By Admin:** {safe_name(admin_name)}\n"
        f"📝 **Content Preview:** {text_content}\n"
        f"⏱ **Posted Time:** `{posted_at}`\n"
