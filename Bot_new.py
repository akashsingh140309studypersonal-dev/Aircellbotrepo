...
# Conversation States
MEDIA_TEXT, OPTION_COUNT, OPTION_DETAILS, CORRECT_ANS, TIMER_INPUT = range(5)
MAT_TITLE, MAT_FILES = range(5, 7)
REQ_TITLE, REQ_FILES = range(7, 9)
RESET_PASSWD = range(9, 10)
ADD_FEATURE, EDIT_FEATURE = range(10, 12)
...
# Setup Dynamic Commands Menu [/]
async def setup_bot_commands(app: Application):
    member_commands = [
        BotCommand("start", "Start the bot / Show main menu"),
        BotCommand("myprofile", "View your profile stats"),
        BotCommand("profile", "View user profile by ID or @username"),
        BotCommand("top", "View Top 30 leaderboard"),
        BotCommand("materials", "Browse & download study materials"),
        BotCommand("reqmaterial", "Request new study materials"),
    ]

    admin_commands = member_commands + [
        BotCommand("newquiz", "Create a new quiz question"),
        BotCommand("upmaterials", "Upload new study materials"),
        BotCommand("apvmaterial", "Approve or reject user requested materials"),
        BotCommand("removematerial", "Remove material by title or ID"),
        BotCommand("cancel", "Cancel current active process"),
    ]

    owner_commands = admin_commands + [
        BotCommand("giveowner", "Add new owner"),
        BotCommand("addfeature", "Add new dynamic code feature"),
        BotCommand("editfeature", "Edit dynamic code feature"),
        BotCommand("recover", "Recover database entry from user text"),
        BotCommand("add", "Add user directly to database"),
        BotCommand("givepts", "Give points to user"),
        BotCommand("removepts", "Remove points from user"),
        BotCommand("users", "Show total registered members"),
        BotCommand("admins", "Show list of active admins"),
        BotCommand("giveadmin", "Promote user to admin"),
        BotCommand("removeadmin", "Demote user from admin"),
        BotCommand("postchannel", "Set channel for posting quizzes"),
        BotCommand("recent", "View recent 10 quiz history"),
        BotCommand("allhistory", "View complete quiz history"),
        BotCommand("resetall", "Reset all stats (Password required)"),
    ]
...
async def editfeature_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists("custom_features.py"):
        with open("custom_features.py", "r") as f:
            current_code = f.read()
        await update.message.reply_text(f"📜 **Current Code:**\n```python\n{current_code}\n```\n\nSend new code to replace existing dynamic code:", parse_mode="Markdown")
    else:
        await update.message.reply_text("📜 No dynamic code exists yet. Send new Python code to create it:", parse_mode="Markdown")
    return EDIT_FEATURE

async def editfeature_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    code = update.message.text
    with open("custom_features.py", "w") as f:
        f.write(code + "\n")
    execute_custom_code()
    await update.message.reply_text("✅ Dynamic feature code replaced and executed successfully!", parse_mode="Markdown")
    return ConversationHandler.END
...
async def recover_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reply or not reply.text:
        await update.message.reply_text("⚠️ Reply to a message containing ID and Username details using `/recover`.", parse_mode="Markdown")
        return
    
    text = reply.text
    ids = re.findall(r'\b\d{7,12}\b', text)
    usernames = re.findall(r'@([a-zA-Z0-9_]+)', text)
    
    if not ids:
        await update.message.reply_text("❌ No valid User ID found in the replied message.")
        return

    uid = int(ids[0])
    uname = usernames[0] if usernames else "NoUsername"
    fname = "Recovered User"

    cursor.execute("INSERT OR REPLACE INTO users (user_id, username, full_name) VALUES (?, ?, ?)", (uid, uname, fname))
    conn.commit()
    await update.message.reply_text(f"✅ User successfully recovered to DB:\n• ID: `{uid}`\n• Username: `@{uname}`", parse_mode="Markdown")
...
async def give_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if new_owner_id not in OWNERS:
            ...
            cursor.execute("INSERT OR IGNORE INTO admins (user_id) VALUES (?)", (new_owner_id,))
            conn.commit()
            await update.message.reply_text(f"👑 Added `{new_owner_id}` as Bot Owner!", parse_mode="Markdown")
        else:
            await update.message.reply_text("⚠️ User is already an owner.")
    except ValueError:
        await update.message.reply_text("❌ Invalid User ID format.")

async def add_user_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return
    ...
    try:
        ...
        fname = " ".join(context.args[2:]) if len(context.args) > 2 else "Added User"
        
        cursor.execute("INSERT OR REPLACE INTO users (user_id, username, full_name) VALUES (?, ?, ?)", (uid, uname, fname))
        conn.commit()
        await update.message.reply_text(f"✅ User `{uid}` successfully added/updated in Database!", parse_mode="Markdown")
    except ValueError:
        await update.message.reply_text("❌ Invalid User ID format.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    menu_text = f"✨ ──────────────────────── ✨\n"
    menu_text += f"👋 Welcome **{safe_name(user.first_name)}**!\n"
    menu_text += f"✨ ──────────────────────── ✨\n\n"
    ...
    if is_owner(user.id):
        menu_text += (
            ...
            "👑 ❯ `/editfeature` • Edit dynamic code\n"
            "👑 ❯ `/recover` • Recover user to DB (reply to user details)\n"
...
async def users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"📊 **Registered Users Count ({len(all_users)}):**\n\n"
    for u in all_users:
...
# Direct admin rights without dynamic user lookup restriction
...
async def post_channel_manager(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        cursor.execute("INSERT OR REPLACE INTO channels (channel_id, title, is_active) VALUES (?, ?, 1)", (ch_input, ch_input))
        conn.commit()
        await update.message.reply_text(f"✅ Current posting channel set to: `{ch_input}`", parse_mode="Markdown")
        return
    cursor.execute("SELECT channel_id, is_active FROM channels")
    rows = cursor.fetchall()
    msg = "📢 **Channel Posting Settings**\n\n"
    buttons = []
    if not rows:
        msg += "No registered channel found.\nUsage: `/postchannel <@channel_username or Chat_ID>`"
    else:
        for ch, active in rows:
            status = "Registered & Active ✅" if active == 1 else "Registered ❌"
            msg += f"• `{ch}` -> {status}\n"
            row_btns = []
            if active == 0:
                row_btns.append(InlineKeyboardButton(f"Set Active: {ch}", callback_data=f"setch_{ch}"))
            row_btns.append(InlineKeyboardButton(f"🗑 Remove {ch}", callback_data=f"remch_{ch}"))
...
async def channel_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_owner(query.from_user.id):
        return
    if query.data.startswith("setch_"):
        ch_id = query.data.replace("setch_", "")
        cursor.execute("UPDATE channels SET is_active = 0")
        cursor.execute("UPDATE channels SET is_active = 1 WHERE channel_id = ?", (ch_id,))
        conn.commit()
        await query.edit_message_text(f"✅ Current posting channel switched to: `{ch_id}`", parse_mode="Markdown")
    elif query.data.startswith("remch_"):
...
async def resetall_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return ConversationHandler.END
    await update.message.reply_text("⚠️ **WARNING:** This will reset ALL user points, attempts, correct, and incorrect statistics to ZERO!\n\nPlease enter the Admin Password to confirm:", parse_mode="Markdown")
    return RESET_PASSWD
...
async def resetall_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if password == "2kAircell":
        ...
        conn.commit()
        await update.message.reply_text("✅ **ALL USER DATA & LEADERBOARD STATS HAVE BEEN RESET TO ZERO!**", parse_mode="Markdown")
    else:
...
async def view_profile(update: Update, context: ContextTypes.DEFAULT_TYPE, target_uid=None, target_uname=None):
    ...
    if not user_data:
        if not target_uid and not target_uname:
            ...
            await update.message.reply_text("❌ User profile not found in database.", parse_mode="Markdown")
            return
    ...
    profile_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"✨ *Keep solving questions to rise on the leaderboard!*"
    )
...
async def top_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top_users:
        await update.message.reply_text("🏆 No users found in leaderboard yet.")
        return
    ...
    for idx, (fname, uname, pts) in enumerate(top_users, 1):
        ...
        if user_is_admin:
            u_disp = format_username(uname)
            msg += f"{idx}. **{s_fname}** ({u_disp}) - `{pts}` pts\n"
        else:
            msg += f"{idx}. **{s_fname}** - `{pts}` pts\n"
...
async def reqmaterials_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📝 **Material Request Mode**\n\nPlease enter the **Title/Subject** for the material you want to request:\n(Type /cancel to abort)", parse_mode="Markdown")
    return REQ_TITLE
...
async def reqmaterials_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📥 Send files/photos/lectures or notes for this request.\n\nWhen completed, send **/done**.", parse_mode="Markdown")
    return REQ_FILES
...
async def reqmaterials_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🎉 Request for **'{safe_name(title)}'** submitted successfully! Admins will review it soon.", parse_mode="Markdown")
...
async def apvmaterial_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📝 **Pending User Material Requests:**\n\nClick to review:"
    buttons = []
    for (title,) in rows:
        title_sub = title[:20]
        buttons.append([
...
async def apvmaterial_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("viewreq_"):
        ...
        for title, file_id, file_type, text_content in rows:
            caption = f"📋 Request Preview: **{safe_name(title)}**\n\n{text_content}"
        ...
        conn.commit()
        await query.edit_message_text(f"✅ Material request approved and published to `/materials`!")
...
async def upmaterials_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📥 Send all files (Photo, Document, Video, Text). Send **/done** when finished.", parse_mode="Markdown")
    return MAT_FILES
...
async def upmaterials_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🎉 Material **'{safe_name(title)}'** saved successfully!", parse_mode="Markdown")
    context.user_data.clear()
    return ConversationHandler.END
...
async def remove_material_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    if deleted > 0:
        await update.message.reply_text(f"🗑️ Removed `{deleted}` file(s) for **'{safe_name(target)}'**.", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"❌ Material **'{safe_name(target)}'** not found.", parse_mode="Markdown")
...
async def materials_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = cursor.fetchall()
    if not rows:
        await update.message.reply_text("📁 No study materials uploaded yet.")
        return
    msg = "📚 **Available Study Materials:**\n\nClick on any button below to download:"
    buttons = []
    for (title,) in rows:
...
async def materials_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("getmat_"):
        ...
        rows = cursor.fetchall()
        if not rows:
            await query.message.reply_text("❌ Material no longer exists.")
            return
        for title, file_id, file_type, text_content in rows:
...
async def get_media_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [InlineKeyboardButton("MCQ (Single / Multi-Choice)", callback_data="type_mcq")],
        [InlineKeyboardButton("Integer/Decimal Numpad", callback_data="type_num")]
    ]
    await update.message.reply_text("Select Type:", reply_markup=InlineKeyboardMarkup(buttons))
    return OPTION_COUNT
...
async def process_option_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    quiz = context.user_data['quiz']
    if quiz.get('type') == "MCQ" and 'max_options' not in quiz:
        try:
...
async def process_option_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if quiz['current_opt'] < quiz['max_options']:
        ...
        await update.message.reply_text(f"Which option is correct? (e.g. A, B or AB for multi-select):")
        return CORRECT_ANS
...
# MCQ Direct Attempt Option Removed
async def publish_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    skip_lb = 1 if time_res == "skip" else 0
    seconds = time_res if not skip_lb else 0

    target_chat = get_active_channel() or update.effective_chat.id
    quiz_id = f"q{int(datetime.datetime.now().timestamp())}"
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Failed to post in target channel `{target_chat}`.\nError: {e}", parse_mode="Markdown")
        return ConversationHandler.END
    ...
    if skip_lb:
        await update.message.reply_text(f"🚀 Question posted to `{target_chat}`! Leaderboard is **SKIPPED** for this question.", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"🚀 Question posted to `{target_chat}`! Rankings will declare in {seconds} seconds.", parse_mode="Markdown")
...
# Quiz Interaction & Immediate Result Alert
async def handle_quiz_clicks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if quiz_id not in active_quizzes:
        await query.answer("⚠️ Quiz expired.", show_alert=True)
        return
    ...
    if prefix == "mcq":
        if user.id in q_data['responses']:
            await query.answer("You have already selected your option!", show_alert=True)
            return
        ...
        if is_correct:
            await query.answer("✅ You are Right! (+10 Pts)", show_alert=True)
        else:
            await query.answer("❌ You are Wrong! (-5 Pts)", show_alert=True)

    elif prefix == "num":
        if user.id in q_data['responses'] and q_data['responses'][user.id].get('submitted'):
...
async def history_list(update: Update, context: ContextTypes.DEFAULT_TYPE, limit=None):
    ...
    for qid, aname, pat, st, skip in rows:
        sk_txt = "Skipped" if skip else "Active"
        msg += f"• `{qid}` | By: **{safe_name(aname)}** | Status: `{st}` | LB: `{sk_txt}`\n"
...
async def force_declare_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if qid in active_quizzes:
        await declare_leaderboard(type('obj', (object,), {'job': type('obj', (object,), {'data': {'quiz_id': qid}})()})())
...
def main():
    ...
    edit_feature_conv = ConversationHandler(
        ...
        states={
            EDIT_FEATURE: [MessageHandler(filters.TEXT & ~filters.COMMAND, editfeature_save)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    ...
    quiz_conv = ConversationHandler(
        ...
        states={
            ...
            TIMER_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, publish_quiz)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    ...
    app.add_handler(CommandHandler("add", add_user_db))
    ...
    app.add_handler(CallbackQueryHandler(channel_callback_handler, pattern="^(setch|remch)_"))
    ...
    app.add_handler(CallbackQueryHandler(handle_quiz_clicks, pattern="^(mcq|num)_"))
