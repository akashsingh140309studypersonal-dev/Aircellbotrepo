...
async def setup_bot_commands(app: Application):
    ...
    owner_commands = admin_commands + [
        ...
        BotCommand("listadmin", "🛡 Show admins list"),
...
async def recover_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.strip():
        await update.message.reply_text("⚠️ **Smart Recover Usage:**\nReply to any text/list message or send text with `/recover`.", parse_mode="Markdown")
        return
    ...
    for uid_str, uname in matches:
        ...
        # Try fetching real info from Telegram API
        real_fname = None
        try:
            chat_member = await context.bot.get_chat(uid)
            real_fname = chat_member.full_name or chat_member.first_name
            if chat_member.username:
                clean_uname = chat_member.username
        except Exception:
            pass
            
        fname = real_fname if real_fname else f"User {uid}"
        
        cursor.execute("INSERT OR REPLACE INTO users (user_id, username, full_name) VALUES (?, ?, ?)", (uid, clean_uname, fname))
        added_list.append(f"• **{safe_name(fname)}** ({format_username(clean_uname)}) - ID: `{uid}`")
        added_count += 1

    conn.commit()
    response = f"✅ **Smart Recovery Complete!**\nTotal `{added_count}` user entries saved:\n\n" + "\n".join(added_list[:30])
    if len(added_list) > 30:
        response += f"\n...and {len(added_list)-30} more."
    await update.message.reply_text(response, parse_mode="Markdown")

# Dynamic Custom Code Handlers
...
async def addfeature_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Dynamic feature code added, registered, and executed successfully!", parse_mode="Markdown")
    return ConversationHandler.END
...
async def get_help_text(user_id, user_name):
    ...
    if is_owner(user_id):
        menu_text += (
            ...
            "👑 ❯ /botchannels • Check Bot channels & rights\n"
            "👑 ❯ /recover • Recover bulk users to DB\n"
            "👑 ❯ /giveowner <ID> • Grant owner rights\n"
            "👑 ❯ /removeowner <ID> • Remove owner rights\n"
            "👑 ❯ /listowner • View list of owners\n"
            "👑 ❯ /givepts <ID/@user> <pts> • Add Points\n"
            "👑 ❯ /removepts <ID/@user> <pts> • Deduct Points\n"
            "👑 ❯ /listadmin • Active admins list\n"
    ...
    if is_admin(user_id):
        menu_text += (
            "🛠 ━━━━ ADMIN CONTROLS ━━━━ 🛠\n"
            "🛠 ❯ /users • Registered members count & list\n"
...
async def remove_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Usage: `/removeowner <user_id>`", parse_mode="Markdown")
        return
    try:
        target_owner = int(context.args[0])
        if target_owner in OWNERS:
            if len(OWNERS) <= 1:
                await update.message.reply_text("❌ Cannot remove the only remaining Owner!")
                return
            OWNERS.remove(target_owner)
            await update.message.reply_text(f"❌ Revoked Owner status from `{target_owner}`.", parse_mode="Markdown")
        else:
            await update.message.reply_text("⚠️ User is not in the Owner list.")
    except ValueError:
        await update.message.reply_text("❌ User ID must be numeric.")
...
async def list_owners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return
    msg = f"👑 **Bot Owners List ({len(OWNERS)}):**\n\n"
    for oid in OWNERS:
        cursor.execute("SELECT full_name, username FROM users WHERE user_id = ?", (oid,))
        row = cursor.fetchone()
        if row:
            msg += f"• **{safe_name(row[0])}** ({format_username(row[1])}) - ID: `{oid}`\n"
        else:
            msg += f"• Owner ID: `{oid}`\n"
    await update.message.reply_text(msg, parse_mode="Markdown")

# Bot Channel & Rights Checker Feature
async def check_bot_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        await update.message.reply_text("📢 No channels registered in DB.")
        return

    status_msg = await update.message.reply_text("🔍 Checking bot status & permissions in channels...")
    res_text = f"📢 **Channels Status & Admin Rights ({len(channels)}):**\n\n"
    ...
    for ch_id, ch_title in channels:
        try:
            ...
            res_text += f"🔹 **{safe_name(chat.title)}** (`{ch_id}`)\n"
            res_text += f"   • Status: `{bot_member.status.upper()}`\n"
            if bot_member.status in ["administrator", "creator"]:
                res_text += f"   • Post Messages: {'✅' if bot_member.can_post_messages else '❌'}\n"
                res_text += f"   • Edit Messages: {'✅' if bot_member.can_edit_messages else '❌'}\n"
            res_text += "\n"
        except Exception as e:
            res_text += f"❌ **{safe_name(ch_title)}** (`{ch_id}`)\n   • Status Error: `{e}`\n\n"
...
async def users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    header = f"📊 **Registered Members Count ({len(all_users)}):**\n\n"
    lines = []
    for u in all_users:
        u_disp = format_username(u[2])
        lines.append(f"• **{safe_name(u[1])}** ({u_disp}) - ID: `{u[0]}`")

    # Split into chunks of 3000 chars to avoid Telegram API 4096 char limit
    chunks = []
...
# Renamed to /listadmin & Fixed User display
async def listadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return
    ...
    for uid, uname, fname in all_admins:
        # Check if missing, fetch live info
        if not fname or fname.startswith("Recovered User"):
            try:
                chat_mem = await context.bot.get_chat(uid)
                fname = chat_mem.full_name or chat_mem.first_name
                if chat_mem.username:
                    uname = chat_mem.username
                cursor.execute("UPDATE users SET full_name = ?, username = ? WHERE user_id = ?", (fname, uname, uid))
                conn.commit()
            except Exception:
                fname = f"User {uid}"

        u_disp = format_username(uname) if uname else "No Username"
        name_disp = safe_name(fname)
...
async def give_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not uid:
        try:
            ...
            await update.message.reply_text("❌ User ID not found.", parse_mode="Markdown")
            return
...
async def publish_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for target_chat in target_channels:
        ...
        cursor.execute("""
            INSERT INTO quiz_history (quiz_id, admin_id, admin_name, chat_id, text_content, correct_ans, posted_at, status, skip_leaderboard)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (quiz_id, user.id, user.full_name, str(target_chat), text_to_send[:50], str(quiz['correct_ans']), now_time_ist, 'pending', skip_lb))
        conn.commit()
    ...
    if prefix == "mcq":
        ...
        if action == "submit":
            ...
            if action in user_selections:
                ...
                await query.answer(f"Deselected Option {action}. Current Selection: {', '.join(sorted(user_selections)) if user_selections else 'None'}")
            else:
                user_selections.append(action)
                await query.answer(f"Selected Option {action}. Current Selection: {', '.join(sorted(user_selections))}")
        ...
        if action in "0123456789.":
            ...
            q_data['temp_inputs'][user.id] = current_val
            await query.answer(f"Deleted. Current: {current_val if current_val else 'Empty'}")
        elif action == "sub":
            if not current_val:
...
async def declare_leaderboard(context: ContextTypes.DEFAULT_TYPE):
    ...
    if q_data.get('skip_lb'):
        ...
        q_data['declared_at'] = dec_time_ist
        return
...
# Cleaned /allhistory command (No force declare, no status, showing Admin, Posted At, Preview & Answer)
async def allhistory_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return

    cursor.execute("SELECT admin_name, posted_at, text_content, correct_ans FROM quiz_history ORDER BY posted_at DESC")
    rows = cursor.fetchall()
    ...
    msg = f"📜 **Complete Quiz History ({len(rows)}):**\n\n"
    lines = []
    for aname, pat, text_content, ans in rows:
        p_time = format_ist_str(pat)
        ans_disp = ans if ans else "N/A"
        lines.append(f"👤 **Admin:** {safe_name(aname)}\n⏱ **Posted At:** `{p_time}`\n📝 **Question:** {text_content}\n🎯 **Answer:** `{ans_disp}`\n───────────────────")

    # Chunking messages for safety
    chunks = []
    current_chunk = msg
    for line in lines:
        if len(current_chunk) + len(line) + 1 > 3800:
