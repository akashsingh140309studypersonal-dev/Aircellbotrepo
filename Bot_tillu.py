...
async def get_help_text(user_id, user_name):
    ...
    if is_admin(user_id):
        menu_text += (
            ...
            "🛠 ❯ /track • Track Question Analytics (Fresh IST)\n"
...
# FRESH ADMIN QUESTION TRACKING FEATURE (/track) IST
async def track_questions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin(user_id):
        return

    active_qids = list(active_quizzes.keys())
    if not active_qids:
        await update.message.reply_text("📌 No active/new questions tracked yet. Post a new question via `/newquiz` to start tracking!", parse_mode="Markdown")
        return

    placeholders = ','.join('?' for _ in active_qids)
    if is_owner(user_id):
        cursor.execute(f"SELECT quiz_id, posted_at, status FROM quiz_history WHERE quiz_id IN ({placeholders}) ORDER BY posted_at DESC", active_qids)
    else:
        cursor.execute(f"SELECT quiz_id, posted_at, status FROM quiz_history WHERE admin_id = ? AND quiz_id IN ({placeholders}) ORDER BY posted_at DESC", [user_id] + active_qids)

    rows = cursor.fetchall()
    if not rows:
        await update.message.reply_text("📌 No new question posted by you yet.")
        return

    buttons = []
    for qid, posted_at, status in rows:
        dt_str = format_ist_str(posted_at)
        status_tag = "🔴 [Live]" if status == 'pending' else "✅ [Declared]"
        buttons.append([InlineKeyboardButton(f"⏱ {dt_str} {status_tag}", callback_data=f"tr_det_{qid}")])

    await update.message.reply_text("📊 **Select Posted Question Timing to view Detailed Audit Report (IST):**", reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")
...
async def track_details_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    qid = query.data.replace("tr_det_", "")
    ...
    if not row:
        await query.edit_message_text("❌ Question tracking data expired or not found.")
        return

    quiz_id, admin_id, admin_name, text_content, posted_at, declared_at, status = row

    if not is_owner(user_id) and admin_id != user_id:
        await query.answer("⚠️ You do not have permission to view other admins' report.", show_alert=True)
        return
    ...
    rep_time_ist = get_ist_now().strftime("%Y-%m-%d %H:%M:%S IST")
    posted_at_ist = f"{format_ist_str(posted_at)} IST"
    declared_at_ist = f"{format_ist_str(declared_at)} IST" if declared_at else "N/A"

    status_str = f"✅ Declared on: `{declared_at_ist}`" if status == 'declared' else f"⏳ Live (Report as of: `{rep_time_ist}`)"
    ...
    if correct_list:
        report += "\n".join([f"• {safe_name(n)}" for n in correct_list[:25]]) + "\n"
    else:
        report += "• None\n"

    report += f"\n❌ **Incorrect Answer Users ({len(incorrect_list)}):**\n"
    if incorrect_list:
        report += "\n".join([f"• {safe_name(n)}" for n in incorrect_list[:25]]) + "\n"
    else:
        report += "• None\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(track_details_callback, pattern="^tr_det_"))
