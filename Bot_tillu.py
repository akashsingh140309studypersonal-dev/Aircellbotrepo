...
# Fresh Tracking Marker Timestamp (Code Run Time)
BOT_START_TIMESTAMP = datetime.datetime.now(IST).strftime("%Y-%m-%d %H:%M:%S")
...
async def setup_bot_commands(app: Application):
    ...
    admin_commands = member_commands + [
        ...
        BotCommand("track", "📍 Track Question Analytics (Fresh)"),
...
# TRACK COMMAND RE-DESIGNED (FRESH RECORD MODE)
async def track_questions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin(user_id):
        return

    cursor.execute("SELECT quiz_id, posted_at, status FROM quiz_history WHERE posted_at >= ? ORDER BY posted_at DESC LIMIT 20", (BOT_START_TIMESTAMP,))
    rows = cursor.fetchall()
    
    if not rows:
        await update.message.reply_text("📌 **No fresh questions posted yet.**\n\nSend a new question via `/newquiz` to start fresh tracking!", parse_mode="Markdown")
        return
    ...
    await update.message.reply_text("📊 **Select Posted Question to view Analytics (Fresh Track):**", reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")

# TRACK CALLBACK FULL FIX
async def track_details_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("Fetching tracking details...")
    user_id = query.from_user.id
    
    if not is_admin(user_id):
        return

    qid = query.data.replace("trdet_", "").strip()
    ...
    if not row:
        await query.message.reply_text("❌ Question tracking record not found.", parse_mode="Markdown")
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(track_details_callback, pattern="^trdet_"))
