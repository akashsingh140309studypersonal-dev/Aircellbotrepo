...
# RESTORED AND PROPERLY FORMATTED HELP MENU FUNCTION
async def get_help_text(user_id, user_name):
    menu_text = f"👋 **Hello {safe_name(user_name)}! Here is your command list:**\n\n"
    
    if is_owner(user_id):
        menu_text += (
            "👑 **OWNER COMMANDS:**\n"
            "• `/addfeature` - Add custom feature code\n"
            "• `/editfeature` - Edit custom code\n"
            "• `/botchannels` - Check bot channels & rights\n"
            "• `/recover` - Smart recover users to DB\n"
            "• `/giveowner` - Grant Owner rights\n"
            "• `/removeowner` - Remove Owner rights\n"
            "• `/listowner` - View all Bot Owners\n"
            "• `/givepts` - Give Points to user\n"
            "• `/removepts` - Remove Points from user\n"
            "• `/admins` - Show admins list\n"
            "• `/giveadmin` - Promote user to Admin\n"
            "• `/removeadmin` - Demote Admin\n"
            "• `/postchannel` - Manage posting channels\n"
            "• `/recent` - View recent 10 quizzes\n"
            "• `/allhistory` - Full Quiz History\n"
    ...
    if is_admin(user_id):
        menu_text += (
            "🛠 **ADMIN COMMANDS:**\n"
            "• `/users` - Total registered members\n"
            "• `/newquiz` - Create new quiz/question\n"
            "• `/track` - Track Question Analytics (48h IST)\n"
            "• `/upmaterials` - Upload study materials\n"
            "• `/apvmaterial` - Review requested materials\n"
    ...
    menu_text += (
        ...
        "• `/top` - Leaderboard Top 30\n"
        "• `/materials` - Download study notes\n"
        "• `/reqmaterial` - Request study material"
    )
