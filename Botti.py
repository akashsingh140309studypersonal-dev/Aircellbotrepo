...
try:
    cursor.execute("ALTER TABLE quiz_history ADD COLUMN correct_ans TEXT")
...
def parse_time_to_seconds(time_str):
    ...
    if unit == 's':
        ...
        return int(val)  # Default to seconds if no unit is specified
    return None
...
async def setup_bot_commands(app: Application):
    ...
    owner_commands = admin_commands + [
        ...
        BotCommand("listadmins", "🛡 Show admins list"),
...
# Fixed Users List Command with Chunking/Pagination
async def users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in all_users:
        ...
        line = f"• {safe_name(u[1])} ({u_disp}) - ID: `{u[0]}`\n"
        
        # Check Telegram message 4000 character limit
...
async def process_correct_ans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏱ Enter ranking timer (e.g. `30s`, `2m`, `1h`) OR type **`skip`** to hide leaderboard completely:", parse_mode="Markdown")
    return TIMER_INPUT
...
# Cleaned up /allhistory (Shows Admin Name, Posting Time, and Correct Answer)
async def allhistory_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        return

    cursor.execute("SELECT admin_name, posted_at, correct_ans FROM quiz_history ORDER BY posted_at DESC")
    rows = cursor.fetchall()
    ...
    msg = "📜 **Full Quiz History:**\n\n"
    for aname, pat, corr in rows:
        p_time = format_ist_str(pat)
        ans_disp = corr if corr else "N/A"
        msg += f"• **Posted By:** {safe_name(aname)} | **Time:** `{p_time}` | **Ans:** `{ans_disp}`\n"

    # Message chunking for history list
    if len(msg) > 3800:
